...
public class csTriangulation : MonoBehaviour {
    ...
    void Start () {
        ...
        for(i=0;i<Vertices.childCount;i++){
            ...
            if(Vertices.GetChild(i).GetComponent<Renderer>().materials[0].name=="amarillo (Instance)") w = 1; else w = 0;
            meshTriangle.Add(Vertices.GetChild(i).position,1);
    ...
    void Update() {
        ...
        for(int i=2;i<tri.Length;i+=3){

            Debug.DrawLine(tri[i-2],tri[i-1],Color.red);
            Debug.DrawLine(tri[i-2],tri[i],Color.red);
            Debug.DrawLine(tri[i-1],tri[i],Color.red);
        }
